...
function formatResponseForEmail(value) {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    const isShopifyLabel = currentLine.startsWith("🔗 ");
    const hasSafeUrlNext = isSafeHttpUrl(nextLine);

    if (isShopifyLabel && hasSafeUrlNext) {
      const label = currentLine.slice(3).trim();
      const safeLabel = escapeHtml(label);
      const safeUrl = escapeHtml(nextLine);
  ...
  <html lang="es">
    ...
    >
      <table
        ...
        cellspacing="0"
        border="0"
        style="background:#f7f2ea; padding:32px 16px;"
      >
        <tr>
          <td align="center">
            <table
              ...
              style="
                ...
                box-shadow:0 18px 45px rgba(69, 49, 25, 0.10);
              "
            >
              <tr>
                <td
  ...
  const contentHtml = `
    ...
    >
      ...
      >
        ...
        ${escapeHtml(responseProvider || "none")}<br />
        <strong>Fuente/modelo:</strong>
        ${escapeHtml(responseModel || "none")}<br />
        <strong>Caso sensible:</strong>
        ${isSensitive ? "Sí" : "No"}
      </div>
    </div>
  ...
  const sensitiveNotice = `
    ...
    >
      <p
        ...
        "
      >
        Tu mensaje será revisado cuidadosamente por nuestro equipo.
        Si estás atravesando una situación urgente, de riesgo o que
        afecta tu seguridad o la de otra persona, busca ayuda inmediata
        de una persona segura, un profesional calificado o una autoridad
        local.
      </p>
    </div>
  `;
    ...
    ? `
      <div
        style="
          ...
          border-radius:20px;
          overflow:hidden;
          box-shadow:0 10px 26px rgba(69, 49, 25, 0.08);
        "
      >
        <div
          style="
        ...
        >
          ...
          <p
            ...
            "
          >
            Queremos acompañarte con una respuesta clara, cercana y
            alineada con la información disponible, para que puedas
            avanzar con confianza dentro de tu experiencia.
          </p>
        </div>
        ...
        >
          <p
            ...
            "
          >
            ✨ Si necesitas orientación adicional, puedes completar
            nuevamente el formulario desde el Centro de Ayuda.
    ...
    : `
      ...
      >
        <p
          ...
          "
        >
          Recibimos tu consulta correctamente. Nuestro equipo podrá
          revisarla y acompañarte con la orientación adecuada según
          tu situación.
        </p>
      </div>
    `;
  ...
  const accessReminder = `
    ...
    >
      <p
        ...
        "
      >
        Si tu consulta está relacionada con el acceso a una App Premium,
        recuerda usar el mismo correo electrónico con el que realizaste
        tu compra.
      </p>
    </div>
  `;
  ...
  try {
    ...
    if (!categoryData) {
      return res.status(400).json({
        ok: false,
        message:
          "Por favor selecciona un tipo de consulta válido.",
        allowedCategories:
          Object.keys(ALLOWED_CATEGORIES)
      });
    }
    ...
    if (!message || message.length < 10) {
      return res.status(400).json({
        ok: false,
        message:
          "Por favor escribe una consulta con más detalle."
      });
    }
    ...
    const isSensitive =
      detectSensitiveMessage(message);
    ...
    if (!isSensitive) {
      try {
        directResponse =
          getDirectResponse(message);

        if (directResponse?.found) {
          console.log(
            "DIRECT_RESPONSE_USED",
            directResponse.source
          );
          ...
          console.log(
            ...
            directResponse.usedFile || "none"
          );

          aiResult = {
            provider: "direct-response",
            model:
              directResponse.usedFile ||
              directResponse.source,
        ...
        console.error(
          "Error generando respuesta directa:",
          error
        );
      }
    } else {
    ...
    return res.status(200).json({
      ...
      directResponse: {
        ...
        used:
          Boolean(directResponse?.found),
        source:
          directResponse?.source || null,
        intent:
          directResponse?.intent || null,
        file:
          directResponse?.usedFile || null
      }
    });
  } catch (error) {
    console.error(
      "Error en api/contact:",
      error
    );
